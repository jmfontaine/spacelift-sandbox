...
EOT
  ...
  after_plan = [<<EOT
if [ -n "$TF_VAR_spacelift_pull_request_id" ]; then curl -d "{\"body\": $(terraform plan -no-color | jq -Rs .)}" -H "Authorization:Token $GITHUB_TOKEN" -s -X POST "https://api.github.com/repos/$TF_VAR_spacelift_repository/issues/$TF_VAR_spacelift_pull_request_id/comments"; fi  
EOT
]
}
